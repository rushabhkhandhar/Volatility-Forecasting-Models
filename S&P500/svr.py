...
OUT_DIR = Path("S&P500/output/SVR_OUTPUT")
...
def load_and_prep(source) -> pd.DataFrame:
    ...
    Price, LogReturn, SqReturn and optional VIX columns.
    """
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # If VIX present, include 1-lag VIX as a feature
    if 'VIX' in df_final.columns:
        df_final['VIX_Lag_1'] = df_final['VIX'].shift(1)
        df_final = df_final.dropna(subset=['VIX_Lag_1'])
        feature_cols = [f"Sq_Return_Lag_{i}" for i in range(1,6)] + [f"Return_Lag_{i}" for i in range(1,6)] + ['VIX_Lag_1']
    else:
...
def main():
    ...
    if HAS_YFINANCE:
        try:
            with open(LOG_PATH, "a") as logf:
                logf.write(f"Attempting to download {default_ticker} and ^VIX from Yahoo Finance ({default_start} to {default_end})...\n")

            nifty_raw = yf.download(default_ticker, start=default_start, end=default_end, progress=False, auto_adjust=False)
            vix_raw = None
            try:
                vix_raw = yf.download('^VIX', start=default_start, end=default_end, progress=False, auto_adjust=False)
            except Exception:
                vix_raw = None
            ...
            if vix_series is not None:
                merged['VIX'] = vix_series.reindex(idx)
            ...
            with open(LOG_PATH, "a") as logf:
                logf.write(f"Downloaded price rows: {len(price_series)}; merged range after reindex: {merged.index.min()} to {merged.index.max()} (rows={len(merged)})\n")
