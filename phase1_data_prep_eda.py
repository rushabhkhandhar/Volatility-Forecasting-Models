...
def main(ticker: str, start_date: str, end_date: str, out_dir: str):
    ...
    # Download India VIX (^INDIAVIX) as an exogenous predictor (if yfinance available)
    vix_col = 'VIX_Close'
    if yf is None:
        print("yfinance not available; skipping VIX download.")
        df[vix_col] = np.nan
        has_vix = False
    else:
        try:
    ...
    plt.title("Nifty 50 Daily Closing Prices")
    plt.xlabel("Date")
    plt.ylabel("Price")
    plt.tight_layout()
    plt.savefig(out_dir / "nifty_prices.png", dpi=150)
    plt.close()

    # Time series plot: Log returns
    plt.figure(figsize=(12, 4))
    plt.plot(log_returns.index, log_returns.values, color="tab:orange", linewidth=0.6)
    plt.title("Nifty 50 Daily Logarithmic Returns")
    plt.xlabel("Date")
    plt.ylabel("Log Return")
    plt.tight_layout()
    plt.savefig(out_dir / "nifty_log_returns.png", dpi=150)
    plt.close()
    ...
    plt.title("21-day Annualized Realized Volatility (Nifty 50)")
    plt.xlabel("Date")
    plt.ylabel("Annualized Volatility")
    plt.tight_layout()
    plt.savefig(out_dir / "nifty_21d_annualized_realized_vol.png", dpi=150)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_dir / "nifty_log_returns_hist.png", dpi=150)
    plt.close()
    ...
    plot_acf((log_returns ** 2).dropna(), lags=40, title="ACF of Squared Log Returns (Nifty 50)")
    plt.tight_layout()
    plt.savefig(out_dir / "acf_squared_log_returns.png", dpi=150)
    plt.close()
    ...
    for p in [
        "nifty_prices.png",
        "nifty_log_returns.png",
        "nifty_21d_annualized_realized_vol.png",
        "nifty_log_returns_hist.png",
        "acf_squared_log_returns.png",
    ]:
        print(f" - {out_dir / p}")
...
if __name__ == "__main__":
    ...
    default_out = os.path.join('outputs', 'phase1')
